...
# Assuming these are defined in config or elsewhere
...
async def set_forcesub(event):
    ...
    # Check if user is sudoer or group owner
    is_owner = await is_group_owner(event, chat_id, user_id)
    if user_id not in SUDOERS and not is_owner:
        await event.reply(to_smallcaps("**You are not authorized to use this command.**"))
        return

    args = event.message.text.split(maxsplit=1)
    if len(args) < 2:
        await event.reply(to_smallcaps("**Usage: /fsub <channel username or ID> or /fsub off to disable**"))
        return
    ...
    if command in ["off", "disable"]:
        ...
        await event.reply(to_smallcaps("**Force subscription has been disabled for this group.**"))
        return
...
async def close_force_sub(event):
    await event.answer(to_smallcaps("Closed!"))
    await event.delete()
...
async def check_forcesub(event):
    ...
    try:
        ...
        forcesub_collection.delete_one({"chat_id": chat_id})
        await event.respond(
            to_smallcaps("**🚫 I'm no longer an admin in the forced subscription channel. Force subscription has been disabled.**")
        )
        await asyncio.sleep(1)
    return False
